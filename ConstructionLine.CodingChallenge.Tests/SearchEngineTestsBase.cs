            ...
            foreach (var size in Size.All)
            {
                ...
                var expectedSizeCount = shirts
                    .Count(s => s.Size.Id == size.Id
                                && (!searchOptions.Sizes.Any() || searchOptions.Sizes.Any(c => c.Id == s.Size.Id))
                                && (!searchOptions.Colors.Any() || searchOptions.Colors.Any(c => c.Id == s.Color.Id)));
            ...
            foreach (var color in Color.All)
            {
                ...
                var expectedColorCount = shirts
                    .Count(c => c.Color.Id == color.Id
                                && (!searchOptions.Colors.Any() || searchOptions.Colors.Any(s => s.Id == c.Color.Id))
                                && (!searchOptions.Sizes.Any() || searchOptions.Sizes.Any(s => s.Id == c.Size.Id)));
